...
RUN mkdir -p static/pyodide && echo '{"packages":[]}' > static/pyodide/package.json
RUN APP_BUILD_HASH=${BUILD_HASH} npx vite build
# Force India AI favicon after build so npm can't regenerate it
RUN cp static/favicon.png /app/build/static/favicon.png
